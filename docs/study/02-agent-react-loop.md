...
**InboundMessage 结构**（参见 `pkg/bus/message.go` 第 14-18 行）：
```go
...
参见 `pkg/agent/agent.go` 第 20-31 行：
...
**Session** 是独立的包（`pkg/session/`），不是 Agent 的内部状态。
...
参见 `pkg/tools/registry.go` 第 47-50 行注释：
